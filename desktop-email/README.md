...
添加邮件后，点击该行的“预览原邮件”即可查看主题、发件人、收件人、抄送、发送时间、完整纯文本正文和附件名称，无需选择输出目录或开始批次。预览只读取源文件，不保存解析结果，也不展开嵌套邮件。关闭按钮或 Esc 返回列表；读取失败时可重试。HTML/RTF 正文转换为纯文本，不呈现原始排版或加载远程图片。
